...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.user.role !== "ADMIN") {
      return NextResponse.json({ error: "Forbidden: Only administrators can create user accounts" }, { status: 403 })
    }

    const data = await request.json()

    // Validate required fields
    const requiredFields = ["username", "password", "email", "fullName", "roleLevel"]
